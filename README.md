...
- 满足其一即可：**含 `-`**（如 `AFL-0011`），或 **同时含字母与数字**（如 `2406NCZ`、`WD25002`），或 **连续纯数字且 ≥3 位**（如 `20230614039`）
- 去首尾 `-` 后长度 ≥ `sku_min_length`；纯字母片段不提取；不足 3 位的纯数字不提取
- **排除容量规格**：`66ml`、`133ml`、`500g` 等（数字+ml/L/g/kg 等），避免与商品编码混淆
- 备注包含 `延迟` 或 `更换` 的订单会直接跳过（不加赠、不审核）
...
| `火石+棉芯  20230614039  通用礼袋  WD25002` | `20230614039` ×1、`WD25002` ×1 |
